...
describe("Rating Slip Lifecycle E2E", () => {
  ...
  describe("Start Rating Slip", () => {
    ...
    it("should search for a player in the new slip modal", () => {
      ...
      cy.get('[role="dialog"]').within(() => {
        cy.get('[role="listbox"], [role="option"], .player-result', {
          timeout: 5000,
        }).should("exist");
      });
    });
    ...
    it("should create a new rating slip successfully", () => {
      ...
      cy.get('[role="dialog"]')
        .find('[role="option"], .cursor-pointer')
        .first()
        .click();
      ...
      cy.get('[role="dialog"]').within(() => {
        cy.get("button")
          .filter(':contains("1"), :contains("2"), :contains("3")')
          .not("[disabled]")
          .first()
          .click();
      });
  ...
  describe("Error Handling", () => {
    it("should show error when starting slip without selecting player", () => {
      ...
      cy.get('[role="dialog"]').within(() => {
        ...
        cy.get('.text-red-500, .text-destructive, [role="alert"]').should(
          "exist",
        );
      });
    });
