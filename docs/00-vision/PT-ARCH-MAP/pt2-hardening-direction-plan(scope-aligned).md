...
- several application surfaces fetch everything on the client. fileciteturn2file1 
...
- worker observability is minimal and failed batches require manual DB inspection. 
...
The report praises PR gates overall, but explicitly calls out that Playwright E2E tests exist and are not part of CI. UI regressions therefore remain undetected before merge. 
...
- Supabase packages are pinned to `latest`, making builds non-reproducible. 
...
## 12. Relationship to Cross-Surface Provenance Governance

The **Cross-Surface Metric Provenance & Truth Governance Plan** should be treated as a **child hardening artifact** under this broader PT-2 hardening direction.

It does **not** replace this hardening plan. It operationalizes one of its most important sub-tracks: **Data Delivery and Freshness Governance**, with secondary impact on **Rendering and Surface Policy**, **Observability**, and **Delivery Confidence**.

### How the two artifacts relate

This hardening plan defines the broad runtime-tightening agenda:
- standardize rendering and surface policy,
- improve observability,
- strengthen release confidence,
- formalize caching, timeout, and data-delivery rules,
- close smaller runtime weak seams.

The cross-surface provenance plan goes one layer deeper and asks:

- what truth-bearing values exist across PT surfaces,
- which are raw vs derived vs interpreted vs historical,
- where each value is computed,
- how fresh each value must be,
- which surfaces consume it,
- whether it can be cached or materialized,
- how it is reconciled when challenged.

In that sense:

- **this hardening plan** governs the **runtime architecture direction**,
- **the provenance plan** governs the **truth semantics and derivation discipline** inside that runtime.

### Direct alignment by hardening area

#### Hardening Area 1 — Rendering and Surface Policy
Cross-surface provenance helps determine **why** different surfaces may require different runtime treatment.

A value’s truth class influences whether a surface should prefer:
- server shaping,
- hybrid delivery,
- live operational queries,
- cached delivery,
- deterministic historical reads,
- snapshot-backed rendering.

Surface classification remains the top-level policy. Provenance sharpens that policy by tying rendering and delivery decisions to the semantics of the values being shown.

#### Hardening Area 2 — Production Observability
The hardening plan calls for better platform observability. The provenance plan extends this into **truth observability**.

In addition to request failures and latency, PT should increasingly observe:
- freshness age of important values,
- recompute or fetch failures for derived metrics,
- divergence between displayed aggregates and source truth,
- reconciliation failures,
- stale or missing compliance/operational indicators.

This is especially important for dashboard and measurement surfaces.

#### Hardening Area 3 — Release Confidence / E2E in CI
The provenance plan helps determine what critical values and journeys must be asserted in end-to-end tests.

It is not enough for a page to load. For important truth-bearing surfaces, CI should increasingly validate that:
- core values render,
- their source dependencies resolve,
- critical interpretations appear when expected,
- fallback/error states are survivable,
- no silent semantic regressions are introduced.

#### Hardening Area 4 — Data Delivery and Freshness Governance
This is the provenance plan’s primary home.

The runtime delivery standard needs policy inputs such as:
- what a value means,
- how often it changes,
- how stale it may become,
- which consumers depend on it,
- whether it should be live, cached, materialized, or snapshotted,
- how it is reconciled.

The cross-surface provenance matrix provides those inputs. Without it, caching and delivery decisions risk becoming implementation convenience rather than governed truth delivery.

#### Hardening Area 5 — Edge Weak Spots and Operational Hygiene
The provenance plan is only indirectly related here. It does not replace hygiene work such as dependency pinning, worker visibility, or DOM-fragile role transport cleanup.

However, these hygiene fixes still support a more trustworthy runtime environment in which truth-bearing surfaces can be delivered consistently.

### Practical governance interpretation

Use the artifacts like this:

- **PT-2 Hardening Direction Plan** = umbrella runtime hardening and standardization plan
- **Cross-Surface Metric Provenance & Truth Governance Plan** = specialized child artifact governing truth-bearing values across surfaces

The provenance plan should therefore be referenced by future work in:
- ADR-039 follow-on measurement work,
- shift and operational dashboard development,
- compliance dashboard work,
- player-derived metric surfaces,
- executive or management summary surfaces,
- caching and runtime delivery standards.

### Bottom line

The hardening plan says PT-2 is structurally strong but weak in runtime discipline.

The provenance plan applies that same hardening logic specifically to **truth delivery**:

- classify truth,
- standardize derivation,
- declare freshness,
- preserve historical semantics where needed,
- and ensure important values can be defended when challenged.

That makes the provenance plan a direct extension of this hardening effort, not a separate initiative.
