...
| Alert notification foundation (deferred) | P2 | See post-C2 slicing | Deferred until persistence + dedup are complete. Channel choice requires separate slice — see §Pilot Containment Rule |
...
- Alert quality telemetry (false-positive rate, acknowledge latency)
- Expected outcome: GREEN (85%) → GREEN (92%)

> **Pilot Containment Rule:** Wedge C notification work is deferred until alert persistence, acknowledgment, and deduplication are complete. External notifications ship as a separate slice. The first channel, if any, is email; Slack is optional and requires explicit customer/operator demand.

**Post C-3 Notification Slices (separate effort, not part of Wedge C completion claim):**

- **C-3A — Notification Foundation:** Channel-neutral delivery policy, severity-based routing rules, outbox + delivery log + retry semantics, destination configuration surface, message templates, audit trail for notification attempts
- **C-3B — Email Adapter:** SMTP/provider configuration, critical-alert-only initial scope, digest/escalation mode deferred
- **C-3C — Slack Adapter (optional):** Only if a pilot/customer actually uses Slack. Not part of the baseline Wedge C completion claim. Requires explicit operator demand, not speculative architecture
...
### Phase 2: Alert Maturity (Week 4-5) — WEDGE C ONLY
...
**Gate:** Wedge C → GREEN (85%) when 2.1–2.3 pass.

### Phase 3: Alert Quality (Weeks 6-7) — WEDGE C ONLY

| # | Action | Effort | Gate |
|---|--------|--------|------|
| 3.1 | Alert context enrichment (activity breakdown + recommended actions) | 2 days | — |
| 3.2 | Alert quality telemetry (false-positive rate, acknowledge latency) | 1 day | — |

**Gate:** Wedge C → GREEN (92%+) when 3.1–3.2 pass. All wedges GREEN.

### Cross-Wedge Polish (Post Wedge C GREEN — Not Part of Wedge C Completion Claim)

| # | Action | Wedge | Effort | Gate |
|---|--------|-------|--------|------|
| P.1 | Loyalty reversal RPC | D | 2 days | Append-only negative entry with source_id reference |
| P.2 | Loyalty snapshot automation (pg_cron or external scheduler) | D | 1 day | — |
| P.3 | Liability snapshot read endpoint + trend API | D | 1 day | — |
| P.4 | Exclusion safety signal wiring (replace NULL stubs) | D | 2-3 days | — |
| P.5 | Slip Detail audit trace panel (UI) | A/B | 2 days | — |

### Post-C3: Notification Slices (Separate Effort)

External notification is **not part of the Wedge C completion claim**. Ships as independent slices after C-3 maturity is validated.

| # | Action | Effort | Prerequisite |
|---|--------|--------|--------------|
| N.1 | Notification foundation: channel-neutral delivery policy, outbox, delivery log, retry, templates, audit trail | 1 week | C-3 complete, alert quality validated |
| N.2 | Email adapter: SMTP config, critical-alert-only scope | 3-4 days | N.1 |
| N.3 | Slack adapter *(optional — only on explicit operator demand)* | 2-3 days | N.1 + confirmed customer need |
...
| **+5 weeks** (Phase 2 complete) | $750K–900K (+ alert persistence + deduplication) | **80%** | Alert infra (persistence, state machine, cooldown) |
| **+8 weeks** (Phase 3 complete) | $900K–1.1M (+ alert quality + operational polish) | **75%** | Context enrichment + quality telemetry + polish |
...
**The single remaining strategic gap is Wedge C's baseline service** — a 2-3 week engineering effort that unlocks 60% of the shift anomaly detection value. Everything else is incremental: alert persistence, deduplication, alert quality, legacy data ingestion, operational polish. External notification channels are a separate post-C3 effort and are not part of the Wedge C completion claim.
