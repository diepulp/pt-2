...
### B. SDLC Alignment (ongoing)
- **Phase 1 (Inception/PRD):** Every scoped slice records which SRM entities/tables it touches plus planned ownership changes.
- **Phase 2 (Architecture):** Produce evidence that proposed RLS, DTOs, and API routes reference SRM-defined ownership; link to ADR-007/ADR-008/ADR-010 where applicable.
- **Phase 3 (Implementation):** Each slice’s Definition of Done includes `supabase gen types` output committed, migrations named per standard, and RLS tests covering the SRM ownership keys.
- **Phase 4–5 (Stabilization/Pilot):** Audits confirm schema ↔ SRM parity (matrix diff), ensuring no tables/columns were introduced outside SRM governance.
