---
id: ADR-053
title: Financial System Scope Boundary
status: Accepted
date: 2026-04-23
owner: Architecture Review
decision_scope: Pilot financial telemetry scope, truth-claim boundary, and reconciliation exclusion
...
canonicalized_from: docs/issues/gaps/financial-data-distribution-standard/decisions/ADR-FINANCIAL-SYSTEM-SCOPE.md
...
* structured operational financial activity (Ledger + Operational classes, per ADR-052)
...
Partial aggregates are permitted **only** when rendered with source, authority, and completeness labels (per ADR-054 §6).
...
A future external reconciliation layer may read from the system via the propagation layer (ADR-054). This ADR defines the **point of integration** (consumer reads outbox events, both classes, with authority labels intact).
...
1. **Surface Rendering Contract** (ADR-054 §6) — any financial surface MUST declare source, authority, completeness. A surface claiming "Total Drop" without those labels is non-conformant.
2. **Propagation contract** (ADR-054) — `origin_label` is immutable in transit; a consumer cannot relabel `Observed` as `Actual`.
...
* Schema-level enforcement of class discriminators (owned by ADR-052)
* Surface rendering mechanics (owned by ADR-054)
