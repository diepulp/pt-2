...
**Acceptance Criteria**
- Single database.types.ts in repo; monolith re-export in use.
...
- Centralize helpers instead of re-declaring them. `ServiceResult`, `generateRequestId`, and other utilities live in `services/shared` so domain modules do not recreate their own success/error builders (`services/table-context/crud.ts:19-70`).
- All domain logic must compose from canonical database types; prohibit hand-written table contracts (`services/player/crud.ts:19-23`). Use DTOs in `types/domains/*` for inputs/outputs and map them explicitly.
...
- Zustand restricted to ephemeral UI state (selection, modal visibility) as demonstrated in `store/player-store.ts`.
