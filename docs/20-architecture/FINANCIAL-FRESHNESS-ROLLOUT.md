...
**P0.1, P0.2, P0.4 can run in parallel.** P0.3 is a stakeholder decision; kick off early so it is settled by the time Phase 2 reaches the MTL slice.
...
| Artifact | EXEC-066 rewrite (`EXEC-066-exemplar-shift-dashboard-rated-buyin.md`); original EXEC-066 archived under `docs/20-architecture/specs/_archive/` |
...
3. Reviewer produces a **Replication Checklist** — a one-page distillation of "what changed, where, in what order" from EXEC-066, written so that the next slice's author can follow it mechanically. This is the actual deliverable that unblocks Phase 2. Without it, Phase 2 is guesswork.
...
| Slice | Fact | Surface | D1 → D2 | Key work | Depends on |
|---|---|---|---|---|---|
| 3.A | `FACT-MTL-PATRON-DAILY-TOTAL` | `components/mtl/compliance-dashboard.tsx` | `player_financial_transaction` (forward bridge) / `mtl_entry` (direct) → `mtl_entry` | Per Replication Checklist. W1 window correctness: operator-driven date navigation qualifies as MANUAL window semantics if D4 is declared INTERVAL; if D4 is LIVE, needs rolling-window treatment. Both paths possible; P0.3 resolves which. | P0.3; Phase 2 `ACTIVE` |
...
| Horizon | Target |
|---|---|
| Week 1 | P0.1, P0.2 land. P0.3 kicked off as stakeholder decision. P0.4 SEC ticket filed. |
| Weeks 2–3 | Phase 1 (exemplar) implementation. Reviewer produces Replication Checklist at exit. |
| Week 4 | Phase 1 merged; registry row `ACTIVE`. Phase 2 slices 2.A / 2.B / 2.C kicked off in parallel. |
| Weeks 5–6 | Phase 2 slices land. Four out of five registry facts are `ACTIVE`. P0.3 decision settled. |
| Weeks 7–8 | Phase 3 (MTL slice) lands. All registered facts `ACTIVE`. |
...
Publication-membership migrations are additive (`ADD TABLE`) and can be reversed with a `DROP TABLE` migration if ever required; no data loss. Realtime hook additions are client-side and remove cleanly.
