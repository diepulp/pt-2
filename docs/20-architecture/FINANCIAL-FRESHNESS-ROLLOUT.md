...
| P0.2 | **Publication-membership audit** — one `pg_publication_tables` query per D2 candidate | **COMPLETE 2026-04-19**: `supabase_realtime` publication has zero user tables; all six D2 tables require ADD-TABLE migration, no backfill path exists. See `REGISTRY_FINANCIAL_SURFACES.md` §P0.2. OVI #5 opened re: `useDashboardRealtime` subscribing `postgres_changes` to unpublished tables. | Phases 1–3 | ≤ 1 day | devops-pt2 / performance-engineer |
...
| 2.B | `FACT-PIT-APPROVALS` | `components/pit-panels/exceptions-approvals-panel.tsx` | `table_fill` / `table_credit` → self | W0 publication **ADD-TABLE** migration for `table_fill` and `table_credit` (P0.2 audit confirmed no ambient membership; previous "backfill" framing was wrong); W1 fix frozen window at `use-exceptions-data.ts:149-150` (§4 E2 violation); W3 LIVE SLA probe under ADDED publication; W4 registry promotion. **W2 `useDashboardRealtime` already subscribes `postgres_changes` on these tables** but cannot actually deliver events until W0 lands (Registry OVI #5) — verify surface freshness reality and decide if existing hook is sound or needs replacement. | Phase 1 `ACTIVE`; P0.2 |
| 2.C | `FACT-SESSION-CUSTODY` | `components/dashboard/pit-dashboard-client.tsx` | `rating_slip` → self | W0 publication **ADD-TABLE** migration for `rating_slip` (P0.2 audit confirmed no ambient membership); W3 LIVE SLA probe under ADDED publication; W4 registry promotion. **W2 `useDashboardRealtime` subscription to `rating_slip` via `postgres_changes`** is currently non-functional pending W0 (Registry OVI #5) — same verification required. Factory-mismatch cleanup (`ratingSlipKeys.activeForTable` vs `dashboardKeys.activeSlips`) noted in slice but deferred to a separate cleanup PR unless it blocks §4 E1 compliance. | Phase 1 `ACTIVE`; P0.2 |

**Why these three are Phase 2 and not later:** 2.A piggybacks on Phase 1's same surface and shares the rolling-window fix. 2.B and 2.C have existing `useDashboardRealtime` `postgres_changes` subscriptions — which the P0.2 audit revealed are currently non-functional at the WAL layer (no publication membership; see Registry OVI #5). The ADD-TABLE migrations W0 ships are therefore not merely registration backfill but the actual activation of those subscriptions. Still the cheapest wins after the exemplar, and they close the contract over the three load-bearing operator surfaces in one cycle.
