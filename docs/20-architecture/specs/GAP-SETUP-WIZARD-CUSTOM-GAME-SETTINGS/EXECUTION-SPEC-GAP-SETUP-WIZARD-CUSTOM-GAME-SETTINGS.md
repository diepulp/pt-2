...
The Setup Wizard (PRD-030) Step 2 provides no UI surface for creating, editing, or deleting custom game settings. The backend CRUD layer is fully built (`services/casino/game-settings-crud.ts`) with DTOs, Zod schemas, mappers, and CHECK constraints. This spec adds the missing server actions and UI to expose that functionality during onboarding.

**Scope**: Phase 1 only (within PRD-030 scope adjustment). No new tables, migrations, or RLS policies required.
...
| Has games | Show game list table + "Add Custom Game" button + seed button if not yet seeded |
