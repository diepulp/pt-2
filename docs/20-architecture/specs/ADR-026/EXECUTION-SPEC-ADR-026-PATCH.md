...
external_dependencies:
  - prd: PRD-000
    ...
    required_for: "compute_gaming_day(casino_id uuid, ts timestamptz) is the canonical function/RPC for gaming-day resolution using casino_settings timezone and gaming_day_start_time"
...
risks:
  - risk: "Schema migration backfill may be slow on large datasets"
    mitigation: "Run against local DB first, validate with db:reset. Backfill uses UPDATE with a WHERE gaming_day IS DISTINCT FROM ... predicate for efficiency and to avoid rewriting unchanged rows."

  - risk: "Race conditions in concurrent visit starts"
    mitigation: "Unique partial index uq_visit_player_gaming_day_active provides database-level safety. RPC uses INSERT + unique_violation exception handling → SELECT to return the canonical row."
...
- `rating_slip_group_id` / explicit slip-to-slip linking across gaming days (deferred). Note: slip rollover (close stale, create new) **is** in scope; cross-day linkage metadata is not.
- Lifetime totals display alongside today totals (deferred question in ADR)

## Architecture Context

**SRM Ownership**: VisitService owns `visit` table (SRM v4.0.0, 2025-12-06; canonical for PT-2)
...
> **Active visit definition (canonical):** `visit.ended_at IS NULL` (there is no separate status flag in scope for this ADR).
...
**Invariant**: `rpc_get_rating_slip_modal_data` must not trust client-supplied casino identifiers; it must derive `v_casino_id` from `current_setting('app.casino_id', true)` after `set_rls_context_from_staff()`. 
...
**Note**: Under Option A, `visit_id` is already gaming-day-scoped; the `pft.gaming_day = v_visit_gaming_day` predicate is kept as defense-in-depth and to protect against legacy rows or accidental cross-day linking.
