...
```sql
-- G1: Context validation (hybrid: use row data if context unavailable)
v_context_casino_id := NULLIF(current_setting('app.casino_id', true), '');
v_context_actor_id := NULLIF(current_setting('app.actor_id', true), '');

-- Use context if available, otherwise derive from row
v_casino_id := COALESCE(v_context_casino_id::uuid, NEW.casino_id);
v_actor_id := COALESCE(v_context_actor_id::uuid, NEW.created_by_staff_id);

-- G2: Tenant invariant check (only if context is set)
IF v_context_casino_id IS NOT NULL THEN
  IF v_casino_id <> NEW.casino_id THEN
...
-- G3: Actor invariant check (only if context is set)
IF v_context_actor_id IS NOT NULL AND NEW.created_by_staff_id IS NOT NULL THEN
  IF v_context_actor_id::uuid <> NEW.created_by_staff_id THEN
    RAISE EXCEPTION 'SECURITY_VIOLATION: Actor mismatch in finance bridge. Context: %, Row: %',
      v_context_actor_id, NEW.created_by_staff_id
      USING ERRCODE = 'P0001';
  END IF;
END IF;
...
**Amount Conversion**:
```sql
-- Finance stores dollars (numeric), telemetry stores cents (bigint)
...
INSERT INTO table_buyin_telemetry (
  casino_id, gaming_day, table_id, visit_id, rating_slip_id,
...
Per SRM v4.9.0 cross-context consumption rules:
