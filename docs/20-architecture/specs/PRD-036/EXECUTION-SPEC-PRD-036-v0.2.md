...
- **ADR-027:** `gaming_table.par_total_cents` exists. Canonical par-set timestamp column exists as `par_updated_at` in types — verify semantics before using for `opening_at` provenance. If the canonical par-set timestamp column is missing or semantics are unclear, set `opening_at = NULL` for Source C (do not block win/loss baseline bootstrapping).
...
- **Rule (no fabricated snapshot IDs):** For Sources C and D, `opening_snapshot_id` MUST remain NULL. Do not mint or reuse synthetic IDs to "look like" a real snapshot.
...
---
## Audit Fold-In (2026-02-19) — Patch List Applied
- Made Source C `opening_at` provenance non-blocking: if par timestamp field is absent/unclear, `opening_at = NULL`.
- Added an explicit rule: no fabricated `opening_snapshot_id` for Sources C/D.
- Clarified semantics: missing closing snapshot ⇒ win/loss stays NULL even if an opening baseline exists.
