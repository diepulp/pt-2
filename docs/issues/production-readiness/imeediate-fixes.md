...
| **6** | **Integrate error tracking (Sentry) [IMPLEMENTED]** | SDK selection, DSN provisioning, Vercel integration vs standalone, PII filtering rules (casino data is sensitive), which events to capture vs ignore, source map upload pipeline, alert routing. This is a small project, not a fix. |
