# ADR-XXX — Financial Event Propagation & Surface Contract
...
scope: Pilot (PT-2)
source: DECISION-CONSOLIDATION.md — D6; outbox mechanics preserved from ADR-FINANCIAL-EVENT-OUTBOX.md (draft)
supersedes: ADR-FINANCIAL-EVENT-OUTBOX.md
depends_on:
- ADR-FINANCIAL-FACT-MODEL (two fact classes, table-first anchoring)
- ADR-FINANCIAL-SYSTEM-SCOPE (no authoritative totals, no reconciliation)
frozen_with:
- ADR-FINANCIAL-FACT-MODEL.md
- ADR-FINANCIAL-SYSTEM-SCOPE.md
- ../actions/SURFACE-RENDERING-CONTRACT.md
purpose: Define how financial events propagate to consumers and how values render at the system boundary without losing their authority semantics.
...
This is the propagation-layer enforcement of ADR-FINANCIAL-SYSTEM-SCOPE D3 (no reconciliation).
...
| `fact_class` | `ledger` \| `operational` (per ADR-FINANCIAL-FACT-MODEL D4). `observation` and `compliance` reserved for taxonomy; not authored in pilot. |
...
* A surface claiming an authoritative total is non-conformant (per ADR-FINANCIAL-SYSTEM-SCOPE D2)
