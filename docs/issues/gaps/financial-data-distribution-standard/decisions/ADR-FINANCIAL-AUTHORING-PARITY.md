# ADR-XXX — Cross-Class Authoring Parity
...
scope: Pilot (PT-2)
extends: ADR-FINANCIAL-EVENT-PROPAGATION.md (tightens D1/D2/D5 with cross-class symmetry invariants)
depends_on:
- ADR-FINANCIAL-FACT-MODEL (two authoring classes)
- ADR-FINANCIAL-EVENT-PROPAGATION (atomic outbox, immutable origin_label)
frozen_with:
...
purpose: Guarantee that the two authoring paths (Class A / Ledger via PFT, Class B / Operational via Grind) produce semantically identical event streams, not just atomically-emitted ones.
