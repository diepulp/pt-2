...
| PlayerService (Compliance) | `player_exclusion` | Authenticated staff in same `casino_id` (Pattern C hybrid SELECT) | INSERT: `pit_boss`/`admin` (Template 2b session-var-only); UPDATE: `admin` only (Template 2b, lift operations only); DELETE: denied | **EXEC-050**: Self-exclusion, regulatory, and trespass exclusion records. INSERT/UPDATE use session-var-only `casino_id` (ADR-030 D4 critical table). UPDATE restricted to lift fields by BEFORE UPDATE trigger. DELETE denied (compliance audit trail). Append-style with immutable creation fields. |
...
**Applies to:** `staff`, `staff_pin_attempts`, `staff_invite`, `player_casino`, `player_exclusion`, `player_financial_transaction`, `visit`, `rating_slip`, `loyalty_ledger`
...
| `player_exclusion` | **A** | Session-var-only (Template 2b) | `rpc_create_player_exclusion`, `rpc_lift_player_exclusion` (planned) | Yes |
...
|---------|------|---------|
| 2.2.0 | 2026-03-27 | **GAP-EXCL-DOC-001**: Added `player_exclusion` to Policy Matrix (PlayerService Compliance context), Template 2b applies-to list, and Category A/B Write Posture cross-reference. Session-var-only INSERT/UPDATE (ADR-030 D4 critical), DELETE denied. |
