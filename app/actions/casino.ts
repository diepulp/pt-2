...
export type CasinoDTO = Pick<CasinoRow, 'id' | 'name' | 'location' | 'address' | 'status' | 'company_id' | 'created_at'>;
export type CasinoCreateDTO = Pick<CasinoInsert, 'name' | 'location' | 'address' | 'company_id' | 'status'>;
export type CasinoUpdateDTO = Partial<Pick<CasinoUpdate, 'name' | 'location' | 'address' | 'company_id' | 'status'>>;
...
export async function getCasinos(options?: { limit?: number; cursor?: string; status?: 'active' | 'inactive' }): Promise<{ casinos: CasinoDTO[]; nextCursor?: string }> {
...
export async function updateCasino(id: string, input: CasinoUpdateDTO): Promise<CasinoDTO> {
  const supabase = await createClient();
  const { data, error } = await supabase
    .from('casino')
    .update(input)
