 ...
 *
 * Rating Slip Pause Route Tests
 *
 * Tests for POST /api/v1/rating-slips/[id]/pause
...
jest.mock('@/services/rating-slip', () => ({
  createRatingSlipService: jest.fn(() => ({
    pause: jest.fn().mockResolvedValue({ id: 'slip-123', status: 'paused' }),
  })),
}));

describe('POST /api/v1/rating-slips/[id]/pause', () => {
  ...
  it('requires Idempotency-Key header', async () => {
    const request = createMockRequest('POST', `/api/v1/rating-slips/${slipId}/pause`);
