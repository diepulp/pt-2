...
import {
  ...
  requestTableCreditSchema,
} from '@/services/table-context/schemas';

/**
 * GET /api/v1/table-context/credits
 *
 * List credits with optional filters (status, gaming_day).
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const query = parseQuery(request, creditListQuerySchema);
    ...
    const result = await withServerAction(
      ...
      async (mwCtx) => {
        ...
        const credits = await listCredits(mwCtx.supabase, {
          status: query.status,
