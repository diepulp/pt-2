 ...
 *
 * Route Handler Tests: /api/v1/loyalty/redeem
 *
 * Tests POST (redeem) endpoint.
...
describe('POST /api/v1/loyalty/redeem', () => {
  ...
  it('requires Idempotency-Key header', async () => {
    const request = createMockRequest('POST', '/api/v1/loyalty/redeem', {
      body: {
        playerId: VALID_PLAYER_ID,
        casinoId: VALID_CASINO_ID,
        points: 50,
        issuedByStaffId: VALID_STAFF_ID,
  ...
  it('returns 201 on new redemption', async () => {
    const request = createMockRequest('POST', '/api/v1/loyalty/redeem', {
      headers: {
