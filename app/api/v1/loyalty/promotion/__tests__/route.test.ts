 ...
 *
 * Route Handler Tests: /api/v1/loyalty/promotion
 *
 * Tests POST (promotion) endpoint.
...
jest.mock('@/services/loyalty', () => ({
  createLoyaltyService: jest.fn(() => ({
    applyPromotion: jest.fn().mockResolvedValue({
...
const VALID_CAMPAIGN_ID = 'PROMO2024';
const VALID_IDEMPOTENCY_KEY = '123e4567-e89b-12d3-a456-426614174002';

describe('POST /api/v1/loyalty/promotion', () => {
  it('exports POST handler', () => {
    expect(typeof POST).toBe('function');
  });

  it('requires Idempotency-Key header', async () => {
    const request = createMockRequest('POST', '/api/v1/loyalty/promotion', {
      body: {
  ...
  it('returns 201 on new promotion', async () => {
    const request = createMockRequest('POST', '/api/v1/loyalty/promotion', {
      headers: {
