...
> **Purpose:** Define how to write PRDs so they are bounded, shippable, and vertically collapsed to the smallest symmetric end-to-end proof when rollout scope spans multiple structural categories; PRDs must not turn into architecture bibles or endless wishlists.
...
When a PRD introduces canonical terms, financial surface values, or epistemic claims (e.g., a new derived metric, authority label, or lifecycle state), it must reference the Semantic Responsibility Layer (`docs/20-architecture/SEMANTIC_RESPONSIBILITY_LAYER.md`). Each such term requires an SRL admission record before the build pipeline will pass GOV-010. This is the semantic parallel to SRM ownership — SRM governs what a service owns; SRL governs what a term means.
