...
**3. Check the proof obligations — all five, never collapsed.** The directive (§14) forbids a single "done" flag. Track separately: **mechanism proof**, **producer-capability proof**, **workflow certification** (does the *real operator workflow* supply the anchors and invoke the canonical path?), **consumer certification** (does the *real surface* render the canonical DTO without recomputing?), **suppression proof** (are competing visible paths removed/unreachable?). The honest edge is almost always workflow or consumer or suppression — name it; do not let RPC-correctness stand in for it. The completion check is a *census*, and the **GitNexus code knowledge graph** (`mcp__gitnexus__*`) is how you make it honest: one `cypher` enumerates the producers of a fact (a fact-bearing name living in two files is the tell), `route_map` inventories the consuming surfaces, and `CALLS` enumeration on a legacy path proves whether it still has live callers (suppression — the strongest live-index signal). The register is the claim; the graph is the audit — extras the graph finds are unmapped nodes (a §12 finding). See `references/gitnexus-evidence.md` for the tool→step map, the smoke-tested census/suppression recipes, the seam-trace and §19-diff verifications, and the load-bearing-vs-blind-spot calibration (consumer/`shape_check` edges are sparse — Read certifies, the graph only points).
...
| `references/gitnexus-evidence.md` | Auditing the completion check / proof obligations with the GitNexus code graph; need the tool→oversight-step map, the producer/consumer-census and suppression Cypher recipes, the seam-trace, the §19 PRD-diff verification, or the operational caveats |
