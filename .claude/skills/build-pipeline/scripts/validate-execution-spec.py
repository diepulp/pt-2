...
def validate_governance(spec: dict[str, Any], spec_content: str) -> tuple[list[str], list[str]]:
    ...
    # Use word-boundary matching to avoid false positives (e.g., "player-import" != "player" table)
    for table, owner in SRM_EXCLUSIVE_OWNERSHIP.items():
        if service and owner != service:
            table_pattern = re.compile(r'(?<![a-zA-Z0-9_-])' + re.escape(table) + r'(?![a-zA-Z0-9_-])')
            ...
            for ws_id, ws in workstreams.items():
                ...
                for output in outputs:
                    if isinstance(output, str) and table_pattern.search(output.lower()):
                        errors.append(
                            f"SRM VIOLATION: {ws_id} modifies '{table}' but it's owned by {owner}, "
                            f"not {service}. Requires SRM coordination or reassignment."
                        )

                # Check description for table mentions (word-boundary match)
                if table_pattern.search(ws_desc) and ("add" in ws_desc or "extend" in ws_desc or "column" in ws_desc):
                    warnings.append(
