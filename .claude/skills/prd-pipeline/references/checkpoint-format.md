...
This document defines the checkpoint file format for PRD pipeline state management.
...
```
.claude/skills/prd-pipeline/checkpoints/{PRD-ID}.json
```

Example: `.claude/skills/prd-pipeline/checkpoints/PRD-003.json`
...
|--------|-------------|-------------|
| `initialized` | Checkpoint created, EXECUTION-SPEC ready | Begin phase 1 |
...
```bash
cat .claude/skills/prd-pipeline/checkpoints/PRD-009.json | jq
```

### Check If Checkpoint Exists

```bash
test -f .claude/skills/prd-pipeline/checkpoints/PRD-009.json && echo "exists" || echo "not found"
```
...
When `/prd-execute PRD-XXX --resume` is invoked:

1. **Load checkpoint**: Read `.claude/skills/prd-pipeline/checkpoints/PRD-XXX.json`
2. **Check status**:
