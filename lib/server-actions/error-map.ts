...
export function mapDatabaseError(error: unknown): MappedError {
  ...
  if (error && typeof error === 'object') {
    ...
    // PostgREST errors
    if (code === 'PGRST116') {
      const domainError = new DomainError('NOT_FOUND', 'Record not found', { details });
      return {
        code: domainError.code,
        message: domainError.message,
        httpStatus: domainError.httpStatus,
    ...
    if (isResultCode(code)) {
      const domainError = new DomainError(code, normalizeMessage(message), { details });
      return {
        code: domainError.code,
        message: domainError.message,
        httpStatus: domainError.httpStatus,
  ...
  // Unknown error type
  const domainError = new DomainError('INTERNAL_ERROR', 'Internal error occurred', {
    details: error,
  });
  return {
    code: domainError.code,
