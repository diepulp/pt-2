  ...
  describe("Query Operations", () => {
    ...
    describe("listByGamingDay()", () => {
      ...
      it("should filter by casino ID", async () => {
        ...
        if (result.success) {
          expect(
            result.data.every((e) => e.casino_id === testCasinoId),
          ).toBe(true);
        }
      });
    });
    ...
    describe("listByPatron()", () => {
      ...
      it("should filter by gaming day", async () => {
        const result = await mtlService.listByPatron(
          testPatronId,
          "2025-10-07",
        );
    ...
    describe("listByCTRThreshold()", () => {
      it("should list entries meeting CTR threshold", async () => {
        ...
        if (result.success) {
          expect(result.data.length).toBeGreaterThan(0);
          expect(
            result.data.every((e) => Number(e.amount) >= 10000),
          ).toBe(true);
        }
      });

      it("should filter by gaming day", async () => {
        const result = await mtlService.listByCTRThreshold(
          5000,
          "2025-10-07",
        );
