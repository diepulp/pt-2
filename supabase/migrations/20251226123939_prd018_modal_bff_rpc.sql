-- ============================================================================
-- Migration: PRD-018 Rating Slip Modal BFF RPC
-- ============================================================================
-- Purpose: Single round-trip RPC for rating slip modal data aggregation
-- Performance: Reduces modal-data endpoint from ~600ms to ~150ms
-- Security: SECURITY INVOKER (inherits caller's RLS context)
-- ADR: ADR-015 (connection pooling), ADR-018 (RPC governance), ADR-020 (Track A)
-- Spec: docs/20-architecture/specs/PERF-001/BFF-RPC-DESIGN.md
-- ============================================================================

-- ============================================================================
-- RPC: rpc_get_rating_slip_modal_data
-- ============================================================================
-- Aggregates modal data from 7 tables in a single database round trip:
--   1. rating_slip - Slip details
--   2. rating_slip_pause - Pause history (for duration calculation)
--   3. visit - Session anchor
--   4. player - Player identity
--   5. player_loyalty - Points balance and tier
--   6. player_financial_transaction - Financial summary
--   7. gaming_table - Table details and active tables
--
-- Returns JSONB matching RatingSlipModalDTO structure
-- ============================================================================
...
COMMENT ON FUNCTION rpc_get_rating_slip_modal_data IS
  'BFF RPC: Single round trip aggregation for rating slip modal display. SECURITY INVOKER (inherits RLS). Returns complete modal DTO as JSONB. Defense-in-depth casino_id validation. See PRD-018, PERF-001 BFF-RPC-DESIGN.md';
...
-- Migration Completed
-- ============================================================================
-- This RPC consolidates 6+ database queries into a single round trip:
--
-- Before (multi-query):
--   Phase A: getById(slip) + getById(visit)
--   Phase B: getTable + getDuration + getPlayer + getFinancial + getActiveTables
--   Phase C: getBalance + evaluateSuggestion + getOccupiedSeats
--   Total: ~600ms p95
--
-- After (BFF RPC):
--   Single RPC call with server-side JOINs and aggregation
--   Target: ~150ms p95
--
