-- Fix start_rated_visit function to get seats_available from gamesettings instead of gamingtable
-- The original function incorrectly tried to access seats_available from gamingtable table,
-- but this column exists in the gamesettings table as it's a game-specific configuration.
...
RETURNS JSON AS $$
  ...
  BEGIN
    ...
    -- FIXED: Get seats_available from game settings, not from gamingtable
