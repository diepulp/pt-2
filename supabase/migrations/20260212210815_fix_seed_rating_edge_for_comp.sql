-- ============================================================================
-- Migration: Fix rating_edge_for_comp for UTH and HCF in seed RPC + existing data
-- Description: Sets rating_edge_for_comp = house_edge for carnival games that were
--              seeded with NULL rating_edge_for_comp.
-- Reference: GAP-SEEDED-GAME-SETTINGS-ORPHANED WS1
-- ============================================================================

-- 1. Fix existing data: set rating_edge_for_comp = house_edge for UTH and HCF
UPDATE game_settings
SET rating_edge_for_comp = house_edge
WHERE code IN ('uth', 'high_card_flush')
  AND rating_edge_for_comp IS NULL;

-- 2. Update the seed RPC to include rating_edge_for_comp for UTH and HCF going forward
...
BEGIN
  ...
  WITH seed_data (code, game_type, name, variant_name, shoe_decks, deck_profile, house_edge, rating_edge_for_comp, decisions_per_hour, seats_available, notes) AS (
    VALUES
      ...
      ('uth',                             'carnival',            'Ultimate Texas Hold ''Em',                          'standard',                                             NULL::smallint, 'standard_52',        2.190,          2.190,          30,      7,      NULL),
      ('high_card_flush',                 'carnival',            'High Card Flush',                                   'optimal baseline',                                     NULL::smallint, 'standard_52',        2.640,          2.640,          50,      7,      NULL)
  ),
  inserted AS (
