-- Migration: PRD-016 Visit Live View RPC
-- Purpose: Single published query for session aggregates that presents operators with a stable "session slip" view.
--
-- Creates rpc_get_visit_live_view(p_visit_id UUID, p_include_segments BOOLEAN, p_segments_limit INTEGER)
-- Returns JSONB with:
-- - Visit metadata (player info, status, started_at)
-- - Current segment (active slip if any: table, seat, status, avg_bet)
-- - Session totals (duration, buy_in, cash_out, net, points, segment_count)
-- - Optional segments array (recent slips with details)
--
-- SECURITY: SECURITY INVOKER (ADR-015 compliance) - respects RLS
-- Session Identity: visit_id is the canonical operator session key
--
-- Usage Examples:
-- - Basic view: SELECT rpc_get_visit_live_view('visit-uuid-here');
-- - With segments: SELECT rpc_get_visit_live_view('visit-uuid-here', TRUE, 10);

-- ============================================================================
-- rpc_get_visit_live_view: Session aggregate query for operators
-- ============================================================================
...
-- Test basic view (no segments):
-- SELECT rpc_get_visit_live_view('visit-uuid-here');

-- Test with segments:
-- SELECT rpc_get_visit_live_view('visit-uuid-here', TRUE, 5);

-- Test cross-casino query (should return NULL due to RLS):
-- SELECT rpc_get_visit_live_view('other-casino-visit-uuid');

-- Test on open visit with active slip:
-- SELECT rpc_get_visit_live_view(id) FROM visit WHERE ended_at IS NULL LIMIT 1;

-- Test on closed visit:
-- SELECT rpc_get_visit_live_view(id) FROM visit WHERE ended_at IS NOT NULL LIMIT 1;
