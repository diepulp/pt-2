-- ============================================================================
-- Migration: Drop 4-param rpc_import_create_batch overload (PostgREST ambiguity fix)
-- Created: 2026-02-26
-- PRD Reference: PRD-039 Server-Authoritative CSV Ingestion Worker
--
-- Problem:
--   PostgREST cannot disambiguate between:
--     rpc_import_create_batch(text, text, text DEFAULT NULL, jsonb DEFAULT '{}')
--     rpc_import_create_batch(text, text, text, jsonb, import_batch_status DEFAULT NULL)
--   When called with 4 named args both signatures match, producing:
--     "Could not choose the best candidate function"
--
-- Fix:
--   Drop the original 4-param overload. The 5-param version with
--   p_initial_status DEFAULT NULL already handles the 4-param case
--   (NULL → 'staging' backward compat logic in function body).
--   Also add DEFAULT values to p_vendor_label and p_column_mapping on
--   the remaining function to preserve full backward compat.
-- ============================================================================

-- ============================================================================
-- 1. DROP the 4-param overload
-- ============================================================================

DROP FUNCTION IF EXISTS public.rpc_import_create_batch(text, text, text, jsonb);

-- ============================================================================
-- 2. REPLACE the 5-param function with DEFAULTs on all optional params
--    (body unchanged from 20260225145409)
-- ============================================================================
...
COMMENT ON FUNCTION public.rpc_import_create_batch(text, text, text, jsonb, public.import_batch_status) IS
  'PRD-037/039: Create import batch with optional initial status. Single canonical function — '
  'p_initial_status DEFAULT NULL gives backward compat (NULL → ''staging''). '
  'Server flow passes ''created''. Derives casino_id/actor_id from context (ADR-024). '
