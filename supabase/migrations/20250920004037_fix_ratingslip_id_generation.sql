-- Fix ratingslip ID generation in start_rated_visit function
-- The ratingslip.id column is of type TEXT and requires an explicit value
-- since it doesn't have a default value like UUID columns
...
RETURNS JSON AS $$
  ...
  BEGIN
    ...
    -- FIXED: Generate explicit TEXT ID for ratingslip since it doesn't have a default
