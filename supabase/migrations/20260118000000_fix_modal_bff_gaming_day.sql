-- ============================================================================
-- Migration: Fix Modal BFF RPC Gaming Day Extraction
-- ============================================================================
-- Issue: ISSUE-CLIENT-GD-001
-- Problem: rpc_get_rating_slip_modal_data extracts gaming_day from rating_slip.start_time
--          instead of visit.gaming_day, ignoring casino's gaming day cutoff
-- Fix: Update RPC to fetch gaming_day from visit table (computed via compute_gaming_day trigger)
-- ============================================================================
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      ...
      'seatsAvailable', COALESCE(t.seats_per_table, 7)
    ) ORDER BY t.label
  ), '[]'::jsonb)
  ...
  FROM (
    ...
    GROUP BY gt.id, gt.label, gt.type, gt.status, gs.seats_per_table
  ) t;
...
COMMENT ON FUNCTION rpc_get_rating_slip_modal_data IS
  'BFF RPC: Single round trip aggregation for rating slip modal display. SECURITY INVOKER (inherits RLS). Returns complete modal DTO as JSONB. Defense-in-depth casino_id validation. Uses gaming_day from visit table (computed via compute_gaming_day trigger). See PRD-018, PERF-001 BFF-RPC-DESIGN.md';
