-- ============================================================================
-- Migration: Fix BFF RPC to include pit_cash_observation in financial summary
-- ============================================================================
-- Issue: CHIPS-TAKEN-NOT-PERSISTED
-- Root cause: rpc_get_rating_slip_modal_data only queried player_financial_transaction
--   for the financial section, ignoring pit_cash_observation records entirely.
--   Chips taken were persisted correctly but never appeared in the modal UI.
--
-- Fix: Replace direct player_financial_transaction query with
--   visit_financial_summary view (single source of truth) which unions
--   player_financial_transaction and pit_cash_observation.
--
-- Dependency: 20260218203729_add_pit_cash_observation_to_visit_financial_summary.sql
-- ============================================================================
...
BEGIN
  ...
  -- ═══════════════════════════════════════════════════════════════════════
  -- FIX (CHIPS-TAKEN-NOT-PERSISTED): Use visit_financial_summary view which
  -- unions player_financial_transaction + pit_cash_observation (dollars→cents).
  -- Previously only queried player_financial_transaction, so pit_cash_observation
  -- amounts (chips taken) were invisible to the modal.
  SELECT
...
COMMENT ON FUNCTION rpc_get_rating_slip_modal_data IS
  'BFF RPC: Single round trip aggregation for rating slip modal display. ADR-024 compliant - uses set_rls_context_from_staff(). SECURITY INVOKER (inherits RLS). Returns complete modal DTO as JSONB. Financial summary now includes pit_cash_observation via visit_financial_summary view. Defense-in-depth casino_id validation. Uses gaming_day from visit table. See PRD-018, PERF-001 BFF-RPC-DESIGN.md';
