-- ============================================================================
-- Migration: ADR-026 BFF RPC Gaming Day Filter
-- ============================================================================
-- Purpose: Update rpc_get_rating_slip_modal_data to filter financial aggregation
--          by gaming_day for defense-in-depth compliance.
--
-- Problem: Financial totals were aggregated by visit_id without gaming_day filter,
--          potentially showing multi-day amounts as "today's" total.
--
-- Solution:
--   1. Fetch gaming_day from visit table (now has gaming_day column from WS1)
--   2. Filter player_financial_transaction by gaming_day (defense-in-depth)
--   3. Return gamingDay from visit (not computed from slip.start_time::date)
--
-- Reference: ADR-026-gaming-day-scoped-visits.md, EXECUTION-SPEC-ADR-026-PATCH.md
-- Security:
--   - Uses v_context_casino_id (derived from context) not p_casino_id (param)
--   - gaming_day filter is defense-in-depth since visit_id is already day-scoped
--
-- Invariant INV-3: Financial aggregations for "today" use gaming_day filter
-- ============================================================================
...
BEGIN
  ...
  WHERE rs.id = p_slip_id
    AND rs.casino_id = v_context_casino_id;
  ...
  WHERE rsp.rating_slip_id = p_slip_id
    AND rsp.casino_id = v_context_casino_id;
  ...
  -- ═══════════════════════════════════════════════════════════════════════
  -- 2. FETCH VISIT AND PLAYER (with gaming_day from visit - ADR-026)
  -- ═══════════════════════════════════════════════════════════════════════
  SELECT
    v.id AS visit_id,
    v.player_id,
    v.gaming_day AS visit_gaming_day,  -- ADR-026: Get gaming_day from visit
    p.first_name,
    p.last_name
  INTO v_visit
  FROM visit v
  ...
  WHERE gt.id = v_slip.table_id
    AND gt.casino_id = v_context_casino_id;
  ...
  IF v_visit.player_id IS NOT NULL THEN
    ...
    WHERE pl.player_id = v_visit.player_id
      AND pl.casino_id = v_context_casino_id;
  ...
  -- ═══════════════════════════════════════════════════════════════════════
  -- 5. AGGREGATE FINANCIAL SUMMARY (ADR-026: Filter by gaming_day - INV-3)
  -- Defense-in-depth: visit_id is already gaming-day-scoped, but we filter
  -- by gaming_day explicitly to protect against legacy rows or edge cases.
  -- ═══════════════════════════════════════════════════════════════════════
  SELECT
  ...
  WHERE pft.visit_id = v_slip.visit_id
    AND pft.gaming_day = v_visit.visit_gaming_day  -- ADR-026: Filter by gaming_day
  ...
  FROM (
    ...
    LEFT JOIN rating_slip rs ON rs.table_id = gt.id
      ...
      AND rs.casino_id = v_context_casino_id
    LEFT JOIN game_settings gs ON gs.casino_id = v_context_casino_id AND gs.game_type = gt.type
  ...
  -- ADR-026: gamingDay now comes from visit.gaming_day (not slip.start_time::date)
  -- ISSUE-EEC1A683: Includes casinoId in slip section for MTL threshold checking
  -- ═══════════════════════════════════════════════════════════════════════
  v_result := jsonb_build_object(
    'slip', jsonb_build_object(
      'id', v_slip.id,
      'visitId', v_slip.visit_id,
      'casinoId', v_context_casino_id,
      ...
      'gamingDay', v_visit.visit_gaming_day,  -- ADR-026: From visit, not slip.start_time::date
...
COMMENT ON FUNCTION rpc_get_rating_slip_modal_data(uuid, uuid) IS
  'BFF RPC: Single round trip aggregation for rating slip modal display. '
  'ADR-024 compliant (context injection). '
  'ADR-026: Financial aggregation filtered by gaming_day for compliance. '
  'ISSUE-EEC1A683: Includes casinoId in slip section for MTL threshold checking.';
